...
from pathfinding_algorithms.AdaptivePPC import AdaptivePurePursuitController
...
class AdaptiveControlSimulation:
    ...
    def run_simulation(self, path):
        ...
        for step in range(self.max_steps):
            ...
            # angular_velocity, linear_velocity = self.controller.compute_steering(
            ...
            # Store target point
            target = self.controller.get_target_point(self.car.x, self.car.y)
            ...
            if distance_to_goal < 0.2:
                print(f"Goal reached after {step+1} steps, time {step*self.dt:.2f}s")
                break
